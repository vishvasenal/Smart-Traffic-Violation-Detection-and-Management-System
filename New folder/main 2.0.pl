...
check_license(L_Num) :-
 ...
 %  Vehicle type validation loop
input_vehicle_type :-
    repeat,
    write('Enter Vehicle Type (bike(light), light, heavy.): '), read(V_Type),
    (vehicle_multiplier(V_Type, _) -> 
        assertz(current_type(V_Type)), ! 
    ; 
        write('Error: Invalid Vehicle Type! Please use bike(light), light, heavy.'), nl, fail
    ).
